...
ARCHIVE_PATH = (
    DIST_DIRECTORY
    / "webhook-notifier.zip"
)

ARCHIVE_MEMBER_NAME = "index.js"
...
class WebhookNotifierPackageError(
    RuntimeError
):
    pass
...
def expected_archive_bytes() -> bytes:
    if not BUNDLE_PATH.is_file():
        raise WebhookNotifierPackageError(
            "Webhook notifier bundle is missing: "
            f"{BUNDLE_PATH}"
        )
...
def verify_archive_contents(
    ...
    with ZipFile(
        ...
        packaged_bundle = archive.read(
            ARCHIVE_MEMBER_NAME
        )

    source_bundle = BUNDLE_PATH.read_bytes()

    if packaged_bundle != source_bundle:
        raise WebhookNotifierPackageError(
...
def check_existing_archive(
    ...
    if not ARCHIVE_PATH.is_file():
        raise WebhookNotifierPackageError(
            "Webhook notifier deployment archive "
            "is missing: "
            f"{ARCHIVE_PATH}"
        )
...
def print_summary(
    ...
    print(
        "Webhook notifier package verified."
    )
    print(
        "Bundle:",
        BUNDLE_PATH,
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "OpsFlow webhook notifier Lambda ZIP."
        )
    )
...
def main() -> int:
    ...
    except WebhookNotifierPackageError as error:
        print(
            f"ERROR: {error}"
        )
